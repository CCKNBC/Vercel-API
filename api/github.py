...
def github_json(user,repo,branch,path):
    if user =='':
        result = 'The user cannot be none!'
    if path =='':
        result = 'The path cannot be none!'
    else:
        try:
            if repo =='':
                repo = 'friends'
            if branch =='':
                branch = 'master'
            requests_path = 'https://github.com/' + user + '/' + repo + '/blob/' + branch + '/' + path
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if branch_reg.findall(path):
            ...
            branch = 'master'
        if path_reg.findall(path):
            path = path_reg.findall(path)[0]
        else:
            path = ''
        data = github_json(user,repo,branch,path)
